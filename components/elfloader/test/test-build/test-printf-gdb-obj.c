...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-printf-gdb-obj.h"
...
TEST_CASE("test-printf-gdb", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_printf_gdb_elf, &env);
    if (ctx == NULL) {
