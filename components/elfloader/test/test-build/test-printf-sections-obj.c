...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-printf-sections-obj.h"
...
TEST_CASE("test-printf-sections", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_printf_sections_elf, &env);
    if (ctx == NULL) {
