...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-printf-O3-obj.h"
...
TEST_CASE("test-printf-O3", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_printf_O3_elf, &env);
    if (ctx == NULL) {
