...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-return-if4-obj.h"
...
TEST_CASE("test-return-if4", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_return_if4_elf, &env);
    if (ctx == NULL) {
