...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-return-if1-obj.h"
...
TEST_CASE("test-return-if1", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_return_if1_elf, &env);
    if (ctx == NULL) {
