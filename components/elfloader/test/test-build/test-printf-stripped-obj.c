...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-printf-stripped-obj.h"
...
TEST_CASE("test-printf-stripped", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_printf_stripped_elf, &env);
    if (ctx == NULL) {
