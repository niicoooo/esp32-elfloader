...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-printf-shortcall-obj.h"
...
TEST_CASE("test-printf-shortcall", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_printf_shortcall_elf, &env);
    if (ctx == NULL) {
