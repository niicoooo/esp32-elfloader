...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-return-if2-obj.h"
...
TEST_CASE("test-return-if2", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_return_if2_elf, &env);
    if (ctx == NULL) {
