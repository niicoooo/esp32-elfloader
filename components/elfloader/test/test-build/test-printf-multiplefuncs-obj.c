...
#include "unity.h"
#include "loader.h"
#include "../payload-build/test-printf-multiplefuncs-obj.h"
...
TEST_CASE("test-printf-multiplefuncs", "[esp32-elfloader]") {

    ELFLoaderContext_t* ctx = elfLoaderInitLoadAndRelocate(payload_build_test_printf_multiplefuncs_elf, &env);
    if (ctx == NULL) {
